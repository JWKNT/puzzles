const divider = '<div class="site-divider puzzle-divider" aria-hidden="true"><img src="https://jehlp.net/site-theme/v2/ornaments/puzzle-transition.png" width="28" height="28" alt=""></div>\n';

// Only the leading, solver-only run is a toolbar. The imported empty anchors
// and paragraphs survive verbatim; prose and post-grid links keep their order.
const emptyAnchor = String.raw`<a>\s*<\/a>`;
const solveAnchor = String.raw`<a\b[^>]*\bhref="[^"]+"[^>]*>(?:SudokuPad|penpa\+)<\/a>`;
const solveParagraph = String.raw`<p\b[^>]*>(?:\s|${emptyAnchor})*${solveAnchor}(?:\s|${emptyAnchor})*<\/p>`;
const solveRun = new RegExp(String.raw`^(?:\s|${emptyAnchor}|${solveParagraph}|${solveAnchor})+`, 'i');

function puzzleActions(html) {
  const run = html.match(solveRun)?.[0];
  if (!run || !new RegExp(solveAnchor, 'i').test(run)) return html;
  return '<nav class="puzzle-actions" aria-label="Solve this puzzle">' + run + '</nav><!-- /puzzle-actions -->' + html.slice(run.length);
}
...
export function puzzleContent({ slug, contentHtml: html }) {
  ...
  return (html.slice(0, boundary) + '<div class="puzzle-sheet">\n' + divider +
    '<div class="puzzle-sheet-body">' + puzzleActions(html.slice(boundary)) +
    '</div><!-- /puzzle-sheet-body -->\n</div><!-- /puzzle-sheet -->')
